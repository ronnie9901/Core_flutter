...
class Bank {
  late String name, balance, eyeColor, gender, company, email, phone, address;
  late int age;
  bool isActive;
...
void main() {
  for (int i = 0; i < fact.length; i++) {
    persondetails p1 = persondetails.formperson(person: fact[i]);
    manas.add(p1);
  }
  for (int i = 0; i < manas.length; i++) {
    print(
        "\n Name : ${manas[i].name} , \n age : ${manas[i].age},\n balance :${manas[i].balance},\n eyecolor :${manas[i].eyeColor},\n gender :${manas[i].gender},\n phone :${manas[i].phone},\n email :${manas[i].email},\n company :${manas[i].company},\n address :${manas[i].address}");
  }
}
